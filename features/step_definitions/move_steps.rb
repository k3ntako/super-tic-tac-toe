...
When(/^I am prompted to make a move$/) do
  ...
  test_cli.fake_user_inputs = ['1', '4', '2', '10'] # first input is for selecting human as opponent
...
Then(/^I should be able to enter an integer$/) do
  ...
  expect(test_cli.triggered_actions[15]).to eq 'get_user_input'
end

And(/^I should see who made the previous move$/) do
  expect(test_cli.displayed_messages[8]).to eq 'Previous Move: X on 2'
end

And(/^I should be able to see the updated board$/) do
  expect(test_cli.displayed_messages[9]).to eq 'nil,X,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil'
end

Then(/^the other player should be prompted to make a move$/) do
  expect(test_cli.displayed_messages[10]).to eq 'Enter a number to make a move in the corresponding square (O\'s turn):'
end

And(/^they should be able to enter an integer$/) do
  expect(test_cli.triggered_actions[21]).to eq 'get_user_input'
end

And(/^they should be able to see the updated board$/) do
  expect(test_cli.displayed_messages[12]).to eq 'nil,X,nil,nil,nil,nil,nil,nil,nil,O,nil,nil,nil,nil,nil,nil'
end
...
When(/^the human player has made a move$/) do
  ...
  select_easy = '1'
  board_width = '5'
  test_cli.fake_user_inputs = [select_computer, select_easy, board_width, '25']
...
Then(/^I should see the board update with the computer move$/) do
  expect(test_cli.displayed_messages[14]).to include 'O'

  square_nine = test_cli.displayed_messages[14].split('').last
