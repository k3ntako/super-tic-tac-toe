  ...
  describe 'create_a_game' do
    ...
    it 'should keep asking for an opponent until the user inputs a valid response' do
      ...
      (mock_cli.fake_user_inputs.length - 1).times do |i|
        base_idx = i * 4
        expect(mock_cli.displayed_messages[base_idx + 3]).to eq 'We got an invalid input, try again!'
        expect(mock_cli.displayed_messages[base_idx + 5]).to eq 'Would you like to play a human or a computer?'
        expect(mock_cli.displayed_messages[base_idx + 6]).to eq 'Enter 1 for human, and 2 for computer:'
      end
